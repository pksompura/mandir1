    ...
    <Link to={`/campaign/${campaign?._id}`}>
      <div className="bg-white rounded-lg overflow-hidden">
        ...
        <div className="p-4">
          ...
          <h3 className="mt-1 text-[17px] font-bold h-[50px] leading-snug line-clamp-2">
            {campaign?.campaign_title}
          </h3>
          ...
          <div className="mt-4">
            ...
            <Link to={`/campaign/${campaign?._id}`}>
              <button className="w-full flex gap-2 items-center justify-center bg-[#d6573d] text-white font-bold py-2 px-4 rounded-full">
                <div className="relative">
                  {campaign?.is_approved ? "Donate Now" : "View Updates"}
                  <span>
                    <CiHeart className="animate-ping absolute -right-8 top-[5px]" />
                  </span>
                </div>
...
const CampaignList = () => {
  ...
  // ✅ RTK Query Fetch
  const { data, isLoading, isFetching } = useGetCampaignsByCategoryQuery({
    category:
      activeCategory === "All"
        ? "All"
        : categories.find((c) => c.name === activeCategory)?._id,
    search: searchTerm,
    page,
    perPage,
  });
  ...
  // ✅ Append new data on scroll
  useEffect(() => {
    if (data?.data?.campaigns?.length) {
      if (page === 1) setAllCampaigns(data.data.campaigns);
      else setAllCampaigns((prev) => [...prev, ...data.data.campaigns]);

      if (data.data.campaigns.length < perPage) setHasMore(false);
    }
  }, [data]);
  ...
  return (
    <div className="w-full lg:w-[1200px] mx-auto px-4 py-8 mt-[68px] relative">
      <h2 className="text-4xl font-bold text-center mb-8 text-gray-700">
        Explore Campaigns
      </h2>
      ...
      <div className="flex justify-center mb-6">
        <input
          ...
          onChange={handleSearch}
          placeholder="Search campaign"
          className="border p-2 rounded-lg w-full md:w-80 focus:outline-none focus:border-orange-500"
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading && page === 1 ? (
          ...
          <div className="col-span-3 text-center">
            <p className="text-gray-500 text-xl mt-10">No campaigns found.</p>
          </div>
        )}
      </div>
      ...
      {showScrollTop && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-8 right-6 bg-[#d8573e] hover:bg-[#b94b33] text-white p-3 rounded-full shadow-lg transition-all duration-300 animate-fade-bounce"
